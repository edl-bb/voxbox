...
## [Unreleased]
- Completed the VoxBox rename across the developer-facing scaffolding: Xcode project, targets, scheme, and source directories are now `voxbox`/`voxboxTests`/`voxboxUITests` (Swift module `voxbox`), the app entry point is `VoxBoxApp`, and the Makefile, CI, SwiftLint config, and release scripts all build/package as VoxBox (`VoxBox.dmg`). Legacy `SpeakType` identifiers are kept only where they're functional: the one-time Application Support migration, the `com.speaktype.defaults` UserDefaults suite (so existing settings survive), and uninstall/cleanup paths for old installs. LICENSE copyright updated to Ed Little with the original SpeakType notice retained; README credits the original project.
